...
def iniciar_juego(jug):
    ...
    while st.session_state.partida.color == "negro":
        st.session_state.partida = mazo.tomarcarta()
    st.session_state.turno = 0
...
def jugar_carta(idx):
    ...
    if carta.numero == "+2":
        siguiente = st.session_state.jugadores[(
            st.session_state.turno + 1) % len(st.session_state.jugadores)]
...
if not st.session_state.juego_iniciado:
    ...
    with col_izq:
        ...
        for i, jugador_otro in enumerate(st.session_state.jugadores):
            if i == turno:
                continue
            st.markdown(
                f"<h4>Jugador {i+1}: {len(jugador_otro.mano)} cartas </h4>", unsafe_allow_html=True)

    with col_centro:
        st.markdown("<h1 style='text-align:center'>Pila</h1>",
                    unsafe_allow_html=True)
    ...
    with col_der:
        turno = st.session_state.turno
        st.markdown(
            f"<h2 style='text-align:center'>Turno actual:</h2> <h3 style='text-align:center'>Jugador {turno + 1}</h3>", unsafe_allow_html=True)
